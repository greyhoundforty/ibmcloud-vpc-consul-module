# Generate random password for Consul encryption if not provided
resource "random_password" "consul_encrypt_key" {
  length  = 32
  special = false
}

# Generate random password for ACL bootstrap token
resource "random_password" "consul_acl_token" {
